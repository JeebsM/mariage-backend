...
class Database {
  constructor() {
    ...
    const admin = require("../config/firebase-config"); // To access Firestore API
  ...
  async createRecordWithId(collection, id, document) {
    const result = await this.firestore.collection(collection).doc(id).set(document);
  ...
  async filter(collection, key, id) {
    console.log(collection,key,id);
    const result = await this.firestore.collection(collection).where(key, "==", id).get();
